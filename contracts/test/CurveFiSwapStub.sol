...
contract CurveFiSwapStub is Base, ICurveFiSwap {
    ...
    function initialize(address[3] memory _coins) public initializer {
        Base.initialize();
        coins = _coins;
    ...
    function remove_liquidity (uint256 _amount, uint256[3] calldata min_amounts) external {
        ...
        for(uint256 i=0; i < coins.length; i++){
            ...
            IERC20(coins[i]).transfer(_msgSender(), amounts[i]);
        }
        token.burnFrom(_msgSender(), _amount);
    }
    function remove_liquidity_imbalance (uint256[3] calldata amounts, uint256 max_burn_amount) external {
        (uint256 fee, bool bonus) = calculateExchangeFee(amounts, true);
        uint256 fullAmount;
        for(uint256 i=0; i< coins.length; i++){
            uint256 balance = balances(int128(i));
    ...
    function balances(int128 i) public view returns(uint256) {
        return IERC20(coins[uint256(i)]).balanceOf(address(this));
    }
    ...
    function calculateExchangeFee(uint256[3] memory diff, bool removeLiquidity) internal view returns(uint256 fullFee, bool bonus){
        uint256 averageAmount = 0;
        uint256[] memory _balances = new uint256[](coins.length);
        for(uint256 i=0; i < coins.length; i++){
            _balances[i] = balances(int128(i));
            averageAmount = averageAmount.add(_balances[i]);
        }
        averageAmount = averageAmount.div(coins.length);
        int256 totalFee;
        for(uint256 i=0; i < coins.length; i++){
